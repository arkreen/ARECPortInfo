...
import { StakingRewardsInfo1, StakingRewardsUser1, StakeTransaction1 } from '../types/schema'
...
const Staking_Rewards_Address = "0xc1dCE2f17362C2De4ab4F104f6f88223e0c28B95"

const DefaultPremiumCap = BigInt.fromString('5000000000000000000000')     // 5000 AKRE 
const DefaultPremiumRate = 200

// event Staked(address indexed user, uint256 amount);
export function handleStake(event: Staked): void {
  let stakeTransaction = StakeTransaction1.load(event.transaction.hash.toHexString())!
  stakeTransaction.type = "Stake"
  stakeTransaction.amountTransaction = event.params.amount
  stakeTransaction.save()

  let stakingRewardsInfo = StakingRewardsInfo1.load("StakingRewardsInfo1")!
  ...
  let stakingRewardsUser = StakingRewardsUser1.load(event.params.user.toHexString())!
...
export function handleStakeRewardPaid(event: RewardPaid): void {
  ...
  let stakingRewardsUser = StakingRewardsUser1.load(stakerUser)!
  ...
  let stakeTransaction = new StakeTransaction1(event.transaction.hash.toHexString())
...
export function handleRewardAdded(event: RewardAdded): void {
  ...
  if (stakingRewardsInfo === null) {
    stakingRewardsInfo = new StakingRewardsInfo1("StakingRewardsInfo1")
...
export function handleRewardStakeUpdated(event: RewardStakeUpdated): void {
  ...
  if (stakingRewardsUser === null) {
    stakingRewardsUser = new StakingRewardsUser1(stakerUser)
